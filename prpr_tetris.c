...
# 141 "tetris.c"
 for (y = 1; y < 23 - 1; y++) {
  for (x = 0; x < 12; x++) {
   if (board[y * 12 + x] - shadow[y * 12 + x]) {
    int color = board[y * 12 + x];
...
void show_online_help(void)
{
 ...
 printf("\e[%d;%dH", start, 26 + 28);
 puts("\e[0mj     - left");
 printf("\e[%d;%dH", start + 1, 26 + 28);
 puts("k     - rotate");
 printf("\e[%d;%dH", start + 2, 26 + 28);
 puts("l     - right");
 printf("\e[%d;%dH", start + 3, 26 + 28);
 puts("space - drop");
 printf("\e[%d;%dH", start + 4, 26 + 28);
 puts("p     - pause");
 printf("\e[%d;%dH", start + 5, 26 + 28);
 puts("q     - quit");
}
...
int sig_init(void)
{
 ...
 sigemptyset(&sa.sa_mask); sigaddset(&sa.sa_mask, 
# 316 "tetris.c" 3 4
2
# 316 "tetris.c"
); sa.sa_flags = 0; sa.
...
# 335 "tetris.c"
 ...
 while (1) {
  ...
  if (c == keys[3]) {
   for (; fits_in(shape, pos + 12); ++points)
    pos += 12;
  }
