...
mod insta_tests {
        ...
        I: Iterator<Item = Token> + std::fmt::Debug + std::clone::Clone,
    {
        fn program(&mut self) -> Program {
            if let Some(program) = &self.parser.program {
                return program.clone();
            }
            let program = self.parser.parser.parse_program(&mut self.token_stream);
            self.parser.program = Some(program);
            return self.parser.program.as_ref().unwrap().clone();
        }
    }
